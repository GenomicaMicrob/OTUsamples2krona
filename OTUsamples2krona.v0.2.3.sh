...
do
	...
	paste krona/$SAMPLE.txt krona/$SAMPLE.percent > krona/$SAMPLE.tmp
	sed -i '1i domain;phylum;class;order;family;genus;species\thits\tpercent' krona/$SAMPLE.tmp
	awk '{print $NF}' krona/$SAMPLE.tmp| awk '{total += $0; $0 = total}1' | sed 1d | sed 1i\Accum% > krona/$SAMPLE.accum # Gets accumulative percentages
	paste krona/$SAMPLE.tmp krona/$SAMPLE.accum > krona/$SAMPLE.tsv
	sed -i 's/;/\t/g' krona/$SAMPLE.tsv
...
rm -f krona/*.dat krona/*.tmp krona/*.txt krona/*.tax krona/*.id krona/*.percent krona/*.accum
